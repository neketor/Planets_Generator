...
def Planet_sys_Generator(name):
    ...
    for belt in range(randint(2, 4)):# Отрисовка поясов, и размещение планет по ним.
        belt = circle_pos(900, 450, 800, 0, r)
        planets_belt = circle_pos(900, 450, randint(1, 3), randint(-30, 30), r)
        for pos in belt:
        ...
        for pos in planets_belt:
            ...
            planet = Planet(generate_alphanum_random_string(randint(4, 10)), planet_diam, randint(1, 100), randint(-120, 120))
            print(planet.temperature)
            planet.color_pl = get_pl_color(abs(planet.temperature), planet.color_pl)
            ellipse(draw, pos[0]-pl_render_diam/2, pos[1]-pl_render_diam/2, pl_render_diam, pl_render_diam, planet.color_pl)
            print(planet.color_pl)
            print(planet)
